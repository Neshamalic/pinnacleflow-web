...
function Pill({ tone = "muted", children, icon = null }) {
  // Usa la piel global .pf-pill {success|warning|danger|info|muted}
  const toneClass =
    { muted: "muted", success: "success", warning: "warning", danger: "danger", info: "info" }[
      tone
    ] || "muted";
  return (
    <span className={`pf-pill ${toneClass}`}>
...
const IconTruck = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <path
      d="M3 7h13v8H3zM16 10h4l1 3v2h-5z"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinejoin="round"
    />
...
const getTenderId = (r) =>
  String(pick(r, ["tender_number", "tender_id", "tender", "Tender", "TENDER_ID"]) || "")
    .trim();
const getTitle = (r) =>
  pick(r, ["tender_title", "title", "category", "product_name", "presentation_name"]) || "—";
...
function StatusBadge({ status }) {
  switch (status) {
    case "rejected":
      return (
        <Pill tone="danger" icon={<IconX />}>
          rejected
        </Pill>
      );
    case "submitted":
      return (
        <Pill tone="info" icon={<IconFile />}>
          submitted
        </Pill>
      );
    case "in_delivery":
      return (
        <Pill tone="warning" icon={<IconTruck />}>
          in delivery
        </Pill>
      );
    case "draft":
    ...
    default:
      return (
        <Pill tone="success" icon={<IconCheck />}>
          awarded
        </Pill>
      );
  }
...
function CoverageBadge({ days }) {
  ...
  return (
    <Pill tone={toneMap[b.tone] || "muted"} icon={<IconClock />}>
      {b.label}
    </Pill>
  );
}
    ...
    <div className="pf-page">
      ...
      <div className="pf-header">
        <div>
          ...
          <p style={{ color: "var(--pf-muted)", fontSize: 14 }}>
            Manage and oversee all CENABAST tenders from registration through delivery tracking.
          </p>
        </div>

        <button type="button" onClick={handleNew} className="pf-btn-primary">
          + New Tender
        </button>
      </div>
        ...
        <div className="pf-table-wrap">
          <table className="pf-table">
            ...
            <tbody>
              {loading && (
                <tr>
                  <td colSpan={8} style={{ color: "var(--pf-muted)", padding: "16px 14px" }}>
                    Loading…
                  </td>
                </tr>
              )}
              ...
              {!loading &&
                tenders.map((t) => (
                  <tr key={t.tenderId}>
                    <td className="whitespace-nowrap font-medium">{t.tenderId}</td>
                    <td>{t.title}</td>
                    ...
                    <td>
                      <CoverageBadge days={t.daysSupply} />
                    </td>
                    <td>{formatCLP(t.totalValue)}</td>
                    <td>
                      <div className="pf-actions">
                        <button onClick={() => handleView(t.tenderId)} className="pf-link">
                          View
                        </button>
                        <button onClick={() => handleEdit(t.tenderId)} className="pf-link">
                          Edit
                        </button>
                      </div>
                    </td>
